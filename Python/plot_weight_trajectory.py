...
'''Usage:
    ...
    python  this_file.py  ***_ave-magnitude.log  # this magnitude log containes the weights of all layers
'''
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
def main():
    ...
    for layer in layers:
        ...
        for i in range(num_col):#np.random.permutation(num_col): # i: column number
            ...
            plt.plot(traj[:,0][::step], pad(traj[:,i+1])[::step], color = color, alpha=0.25, label = layer+"-"+str(i), linewidth = 0.2)
